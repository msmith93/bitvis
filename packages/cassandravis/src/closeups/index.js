...
export function closeUpForNode(op, nid, cluster) {
  ...
  if (isWrite(op)) {
    ...
    if (op.step === idx.write && p.replicas.includes(nid) && !p.down.includes(nid))
      return 'writepath'
    if (op.step === idx.ack && nid === COORDINATOR) return 'quorum'
  } else if (op.type === 'get') {
    const idx = getIdx(p)
    if (idx.repair >= 0 && op.step === idx.repair && nid === COORDINATOR) return 'readrepair'
...
export function buildCloseUp(cu, { op, base, derived }) {
  ...
  switch (cu.kind) {
    ...
    case 'writepath':
      return writepath.build(p, cu.node, base.nodes[cu.node], op.type)
    case 'readrepair':
