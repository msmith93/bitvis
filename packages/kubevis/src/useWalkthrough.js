...
// advance when their `advanceOn` predicate observes the user's real action,
// so the effects below intentionally run on every render rather than diffing
...
export function useWalkthrough(snapshot, actions) {
  ...
  // Fire onShow once per step, at the moment the step first becomes visible.
  // A step can be entered long before its waitFor lets it render — the
  // observe steps are entered when their op starts but only show once the
  ...
  // script (e.g. types the kubectl command instead of clicking the preset)
  // still moves the tour.
  useEffect(() => {
